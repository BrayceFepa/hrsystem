  ...
  UserPersonalInformation.findOne({
    ...
    if (!user) {
      UserPersonalInformation.create(userPersonalInformation)
        .then((data) => {
          // Clear personal information cache
          clearCache("/api/personalInformations");
          res.send(data);
        })
        .catch((err) => {
          res.status(500).send({
            message:
              err.message ||
              "Some error occurred while creating the UserPersonalInformation.",
          });
        });
    } else {
      res
        .status(403)
        .send({ message: "Personal Information already exists for this User" });
    }
  });
};
...
exports.findAll = (req, res) => {
  UserPersonalInformation.findAll({
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message ||
          "Some error occurred while retrieving User Personal Informations.",
      });
    });
};
  ...
  UserPersonalInformation.findAll({ where: { userId: userId } })
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message ||
          "Some error occurred while retrieving personal information.",
      });
    });
};
...
exports.deleteAll = (req, res) => {
  UserPersonalInformation.destroy({
    ...
    .then((nums) => {
      ...
      clearCache("/api/personalInformations");
      res.send({
        message: `${nums} User Personal Informations were deleted successfully!`,
      });
    })
    .catch((err) => {
      res.status(500).send({
        message:
          err.message ||
          "Some error occurred while removing all User Personal Informations.",
      });
    });
};
